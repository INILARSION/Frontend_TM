...
def handle_makros(program_string, session_id):
    ...
    p = run([base_path / "binaries/macro_compiler", file_name, result_file], stdout=PIPE, encoding="ascii", stderr=PIPE)
...
def handle_multitape_compiler(request_dict, session_id):
    ...
    create_file(program_file, request_dict["program"])
    create_file(tape_file, request_dict["tape"])
    p = run([base_path / "binaries/multitape_compiler", tape_file, program_file], stdout=PIPE, encoding="ascii", stderr=PIPE)
    remove_file(program_file)
    remove_file(tape_file)
    return {
        "Returncode": p.returncode,
...
def handle_high_level_tm(request_dict, session_id):
    program_file = str(base_path / "binaries/highlevel_tm_prog") + session_id
    output_file = str(base_path / "binaries/highlevel_tm_out") + session_id
    create_file(program_file, request_dict["program"])
    p = run([base_path / "binaries/high_level_compiler", program_file, output_file], stdout=PIPE, encoding="ascii", stderr=PIPE)
    result = {
        "Returncode": p.returncode,
        "Error": p.stdout,
    }
